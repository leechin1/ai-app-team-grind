...
-- WITHOUT pgvector (for compatibility)
-- Can add embeddings later when pgvector is available
-- ============================================
...
-- ============================================
-- DOCUMENT CHUNKS TABLE (For RAG without vectors)
-- Store chunks for basic text search until pgvector is available
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_chunks_source ON public.document_chunks(source_type, source_id);
-- Full text search index on content
CREATE INDEX IF NOT EXISTS idx_chunks_content_fts ON public.document_chunks USING gin(to_tsvector('english', content));
...
-- Function to search chunks using full-text search (alternative to vector search)
...
-- COMPLETE!
-- ============================================
-- This schema works WITHOUT pgvector
-- Uses PostgreSQL full-text search instead
-- Can migrate to pgvector later when available
--
-- Features included:
-- ✅ All core tables (10 tables)
-- ✅ Full-text search for ChatIQ context
-- ✅ document_chunks instead of embeddings
-- ✅ search_chunks() function for text search
