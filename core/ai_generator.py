...
class AIContentGenerator:
    ...
    def generate_flashcards(
        ...
        print(f"✅ Gerados {len(flashcards)} flashcards em {generation_time:.1f}s")
        ...
        return FlashcardGenerationResponse(
            ...
            content_difficulty_assessment=self._assess_content_difficulty(flashcards),
            warnings=warnings,
...
"""
    ...
    def generate_quiz(
        ...
        print(f"📝 Gerando {num_questions} questões de quiz...")
        ...
        for i, q_data in enumerate(questions_data):
            try:
                ...
                print(f"   ⚠️  Questão {i+1} inválida: {str(e)[:80]}")
        
        if len(questions) == 0:
            raise ValueError("Nenhuma questão válida foi gerada")
        
        print(f"✅ Geradas {len(questions)} questões válidas")
    ...
    def _call_gemini_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_msg = str(e)
                print(f"   ⚠️  Tentativa {attempt + 1}/{max_retries} falhou: {error_msg[:80]}")
